...
void HashTableDictionary::printHistogram(){
	
	struct hist{
		int run_length;
		int count;
		inline bool operator< (const hist& rhs) {return run_length < rhs.run_length;}
		inline bool operator> (const hist& rhs) {return run_length > rhs.run_length;}
	
	};
	int length = 0;
	
	std::vector<hist> histogram;
	for(size_t i = 0; i < TABLE_SIZE; i++){
			...
			input.run_length = length;
			input.count = 0;
			for(size_t j = 0; j < histogram.size(); j++){
				if(histogram.at(j).run_length == input.run_length){
					histogram.at(j).count++;
					contains = true;
				}
			}
			if(!contains && input.run_length > 0){
				input.count = 1;
	...
	//sort histogram by run length
	std::sort(histogram.begin(), histogram.end());
	
	//print histogram header
	std::cout<<"run_length,count\n";
	
	//print histogram
	for(auto q  : histogram){
		std::cout<<q.run_length << "," << q.count << std::endl;
	}
...
void HashTableDictionary::printMask(ELEMENT_STATUS es) {
    for(size_t i = 0; i < TABLE_SIZE; i++) {
        if(hashTableMask.at(i) == USED)
            inRed(es == USED || es == ALL ? '-' : ' ');
        else if (hashTableMask.at(i) == AVAILABLE)
            inCyan(es == AVAILABLE || es == ALL ? '-' : ' ');
        else if( hashTableMask.at(i) == DELETED)
            inYellow(es == DELETED || es == ALL ? '-' : ' ');
        else {
